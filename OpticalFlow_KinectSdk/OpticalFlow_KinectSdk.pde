...
//      shader.uniformTexture("tex_ext"   , opticalflow.tex_frames.src);
  ...
  depthImg = new PImage(512, 424);
    ...
    //movie = new Movie(this, "Pulp_Fiction_Dance_Scene.mp4");
    //movie.loop();
  ...
  public void draw() {
    ...
    if( 1==1){
      //movie.read();
        ...
        int maxDepth =  3000; //4.5m
        int[] rawDepth = kinect.getRawDepthData();
        
        
        for (int i=0; i < rawDepth.length; i++) {
          if (rawDepth[i] > maxDepth || rawDepth[i] == 0) {
      ...
      int movie_w = 512;//kinect2.getDepthImage().width;
      int movie_h = 424;//kinect2.getDepthImage().height;
